...
import {
  View, Clipboard, Platform, ToastAndroid, AlertIOS, StyleSheet, Text
} from 'react-native';
...
class SocialShare extends Component {
  ...
  shareLinkWithShareDialog() {
    const contentShareFB = {
      contentType: 'link',
      contentUrl: 'http://tntechs.com.vn',
      contentDescription: this.props.title
    };
    ShareDialog.canShow(contentShareFB).then(
      function(canShow) {
        if (canShow) {
          return ShareDialog.show(contentShareFB);
        }
      }
    ).then( 
      function(result) {
  ...
  render() {
    ...
    return (
      ...
      <Text style={{ color: 'transparent', fontSize: 0, backgroundColor: 'transparent' }}>
        click to open socical share
      </Text>
            ...
            setTimeout(() => {
              if (typeof shareOptions.url !== undefined) {
                ...
                if (Platform.OS === 'android') {
                  ToastAndroid.show('Link has been copied', ToastAndroid.SHORT);
                } else if (Platform.OS === 'ios') {
                  AlertIOS.alert('Link has been copied');
                }
              }
            }, 300);
          }}
